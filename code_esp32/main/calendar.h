...
public:
    ...
    CalendarEvent(const std::string& description, int beginning_day, int beginning_month,
                  int ending_day, int ending_month,
                  int beginning_hour, int ending_hour,int beginning_minutes, int ending_minutes,bool vide)
        : description(description), beginning_day(beginning_day), beginning_month(beginning_month),
          ending_day(ending_day), ending_month(ending_month),
          beginning_hour(beginning_hour), ending_hour(ending_hour),beginning_minutes(beginning_minutes), ending_minutes(ending_minutes) ,vide(vide){}

    // Constructor with message (parses the string to extract event details)
    CalendarEvent(std::string message);

    // Accessor methods to retrieve event details
    std::string getDescription() const { return description; }  // Get event description
    int getBeginningDay() const { return beginning_day; }  // Get event beginning day
    int getBeginningMonth() const { return beginning_month; }  // Get event beginning month
    int getEndingDay() const { return ending_day; }  // Get event ending day
    int getEndingMonth() const { return ending_month; }  // Get event ending month
    int isVide() const { return vide; }  // Check if event is empty
    int getBeginningHour() const { return beginning_hour; }  // Get event beginning hour
    int getBeginningMinutes() const { return beginning_minutes; }  // Get event beginning minute
    int getEndingHour() const { return ending_hour; }  // Get event ending hour
    int getEndingMinutes() const { return ending_minutes; }  // Get event ending minute
...
public :
    ...
    Now(std::string message);

    // Accessor methods to retrieve current time details
    int getDay() const { return day; }  // Get the current day
    int getMonth() const { return month; }  // Get the current month
    int getYear() const { return year; }  // Get the current year
    int getHour() const { return hour; }  // Get the current hour
    int getMinutes() const { return minutes; }  // Get the current minute
